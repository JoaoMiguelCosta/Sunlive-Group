...
import styles from "./DomesticJourneyPlanner.module.css";
...
export default function DomesticJourneyPlanner({ panel = {}, cta = null }) {
  ...
  const proofPoints = Array.isArray(panel?.proofPoints)
    ? panel.proofPoints.map((point) => String(point).trim()).filter(Boolean)
    : [];

  const proofListAriaLabel =
    panel?.ui?.proofListAriaLabel ?? "Vantagens do planeamento";
