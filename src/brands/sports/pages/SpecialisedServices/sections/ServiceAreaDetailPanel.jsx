import styles from "./ServiceAreaDetailPanel.module.css";

function hasItems(value) {
  return Array.isArray(value) && value.length > 0;
}

export default function ServiceAreaDetailPanel({
  id,
  eyebrow,
  title,
  summary,
      ...
      <div className={styles.body}>
        {hasHighlights ? (
          <section className={styles.mainInfo}>
            <p className={styles.blockLabel}>Destaques</p>

            <ul
              className={styles.highlights}
              aria-label={highlightsAriaLabel || title || "Destaques"}
            >
              {highlights.map((item) => (
